...
public class SDictController extends BaseController {
    ...
    public Result getTypeList(@RequestBody PageQuery pageQuery){
        startPage(pageQuery);
        SysDict sDict = getParam(pageQuery, SysDict.class);
        List<SysDict> sDictPageInfo = sDictService.selectSDictTypeList(sDict);
        return getPageInfo(sDictPageInfo);
    }

    @GetMapping("/type/{dictId}")
    public Result getTypeInfo(@PathVariable String dictId){
        SysDict sDict = sDictService.selectSDictById(dictId, Constant.DICT_TYPE);
        return ResultGenerator.returnCodeMessage(BussinessCode.RESULT_GLOBAL_SUCCESS,sDict);
    }

    @PostMapping("/type")
    public Result addDict(@RequestBody SysDict sDict){
        // 判断code是否唯一
    ...
    @PutMapping("/type")
    public Result updateType(@RequestBody SysDict sDict){
        if (sDictService.checkDictTypeUnique(sDict)){
    ...
    public Result getDataList(@RequestBody PageQuery pageQuery) {
        startPage(pageQuery);
        SysDict sDict = getParam(pageQuery, SysDict.class);
        List<SysDict> dictPageInfo = sDictService.selectSDictList(sDict);
        return getPageInfo(dictPageInfo);
    }

    @GetMapping("/data/type/{sDictType}")
    public Result getDictByType(@PathVariable String sDictType){
        List<SysDict> sDicts = sDictService.selectSDictByType(sDictType);
        if (StringUtils.isNull(sDicts)){
            sDicts = new ArrayList<SysDict>();
        }
    ...
    public Result getDataInfo(@PathVariable String dictId){
        SysDict sDict = sDictService.selectSDictById(dictId,Constant.DICT_DATA);
        return ResultGenerator.returnCodeMessage(BussinessCode.RESULT_GLOBAL_SUCCESS,sDict);
    }

    @PostMapping("/data")
    public Result addDictData(@RequestBody SysDict sDict){
    ...
    @PutMapping("/data")
    public Result updateDict(@RequestBody SysDict sDict){
        if (StringUtils.isEmpty(sDict.getCode())){
            return error("字典编码不能为空！");
        }
        // 判断code是否唯一
